...
Event check-in for use with [Breeze CHMS](https://www.breezechms.com/).

Used at [Sovereign Grace Church - Woodstock](https://sgcatlanta.org/) for faster check-in,
particularly when Breeze CHMS is unavailable or degraded.

It uses a simpler setup - only tablets connected via Bluetooth to commodity thermal label printers.
It does not require a separate print station.

It also works in a purely offline mode.
If the Breeze APIs are available, it will use them for check-in;
...
It requires:
- One or more Android tablets or phones with Android 12 (API 31) or higher.
